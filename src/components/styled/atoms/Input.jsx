...
const Input = styled.span`
	...
	& select, input, textarea, & p#title {
        ...
        // If this is a checkbox, set the order to 1
        ${ ( { $type } ) => $type == 'checkbox' && `
            order: 1;
            height: 1rem;
...
export default ( { onChange, type, label, info, $highlight, id, title, onClick, options, validate, error, verbose=false, validation_delay=2000, ...props } ) => {
    ...
    const { placeholder, value, checked, ...parent_props } = props
    const child_props = { placeholder, value, checked }
    ...
    return <Input $type={ type } onClick={ onClick } $highlight={ $highlight } { ...{ ...parent_props, $has_content: value?.length > 0 } } $valid={ valid } >
